...
def resolve_source(source, release=None, collections=None):
    ...
    if not isinstance(source, str) or not source.strip():
        raise ValueError("source must be a data source name or URL")
    source = source.strip()
    if "://" in source:
        if release is not None or collections is not None:
            raise ValueError("release and collections require a named data source")
        remote = RemoteSpec.parse(source)
        return remote, ("",), {"url": remote.url}
    descriptor = get_source(source)
    if release is None:
        raise ValueError(f"Source {source!r} requires a release; "
                         f"choose from {', '.join(descriptor.releases)}")
    if release not in descriptor.releases:
        raise ValueError(f"Unknown release {release!r} for {source}; "
                         f"choose from {', '.join(descriptor.releases)}")
    definition = descriptor.releases[release]
    if isinstance(collections, str):
        collections = [collections]
    if collections is not None and not isinstance(collections, (list, tuple)):
        raise ValueError("collections must be a name or a list of names")
    selected = []
    for name in collections or ():
        if not isinstance(name, str) or name not in definition.collections:
            raise ValueError(f"Unknown collection {name!r} for {source}/{release}; "
                             f"choose from {', '.join(definition.collections)}")
        if name not in selected:
            selected.append(name)
    roots = {path for name in selected for path in definition.collections[name]}
    # Parent roots cover nested roots; do not crawl overlapping trees twice.
    roots = tuple(path for path in sorted(roots) if not any(
        path.startswith(parent + "/") for parent in roots if parent != path))
    return definition.remote(), roots or ("",), {
        "name": source, "release": release, "collections": selected,
        "url": definition.url,
    }
